...
We can consolidate hypothesis testing pairs into a privacy loss distribution (PLD) while still fully capturing privacy characteristics \cite{DworkR16}. To formalize the PLD for $(P, Q)$, we require the likelihood ratio (Radon--Nikodym derivative) $\frac{dP}{dQ}$. Formally speaking, this requires absolute continuity. However, by extending likelihood ratios with $\infty$ as needed, we can allow mechanisms that violate absolute continuity such as the $(\eps, \delta)$-RR mechanism.

\begin{definition}
	Let $P$ and $Q$ be distributions on a common probability space. The PLD of $(P, Q)$, denoted $\pld(P \parallel Q)$, is the distribution over $\bR \cup \{\infty\}$ of $\log(\frac{dP}{dQ}(\omega)), \omega \sim P$.
\end{definition}
