...
\usepackage[style=alphabetic,maxalphanames=4,minalphanames=3, maxbibnames=10, backend=biber, natbib=true]{biblatex}
\addbibresource{refs/zotero_export.bib}
\addbibresource{refs/manual_refs.bib}
\addbibresource{refs/biblio.bib}

\DeclareSourcemap{
	\maps[datatype=bibtex, overwrite]{
		\map{
			% For any entry of type 'article'...
			\pertype{article}
			% ...find the 'urldate' field and remove it.
		...
		\map{
			% For any entry that has an `eprint' field
			\step[fieldsource=eprint,fieldset=urldate, null]
		}
	}
}
...
\begin{document}
	...
	\input{sections/adversary_theory}
	\input{sections/natural_filter_characterization}
	\input{sections/implications_special_cases}
	% \input{sections/pld_filter_bound}
	\input{sections/well_ordering_proof}
